...
TERM_CONSTEXPR CharItr utf8_to_utf32(const CharItr first, const CharItr last, std::int32_t* out){
	...
	switch(id){
		...
		break; case Private::Identifier::Bytes2: 
			if(std::distance(first, last) >= 2){
				if((first[1] & 0xC0) == 0xC0){
					*out = (static_cast<std::int32_t>(first[0] & 0x1F) << 6) 
							| static_cast<std::int32_t>(first[1] & 0x3F);
					return first + 2;
				}
			}
		
		break; case Private::Identifier::Bytes3: 
			if(std::distance(first, last) >= 3){
				if((first[1] & 0xC0) == 0xC0 && (first[2] & 0xC0) == 0xC0){
					*out = (static_cast<std::int32_t>(first[0] & 0x0F) << 12)
							| (static_cast<std::int32_t>(first[1] & 0x3F) << 6)
							| (static_cast<std::int32_t>(first[2] & 0x3F));
					return first + 3;
				}
			}
						
		break; case Private::Identifier::Bytes4: 
			if(std::distance(first, last) >= 4){
				if((first[1] & 0xC0) == 0xC0 && (first[2] & 0xC0) == 0xC0 && (first[3] & 0xC0) == 0xC0){
					*out = (static_cast<std::int32_t>(first[0] & 0x07) << 24)
							| (static_cast<std::int32_t>(first[1] & 0x3F) << 12)
							| (static_cast<std::int32_t>(first[2] & 0x3F) << 6)
							| (static_cast<std::int32_t>(first[3] & 0x3F));
					return first + 4;
				}
			}
			
		break; case Private::Identifier::Unsupported: break;
		break; default: break;
	}	
	return first;
}
