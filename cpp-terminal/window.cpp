...
char32_t Term::Window::get_char(size_t x, size_t y) {
    ...
    Term::fg Term::Window::get_fg(size_t x, size_t y) { return m_fg[(y - 1) * w + (x - 1)]; }

    Term::bg Term::Window::get_bg(size_t x, size_t y) { return m_bg[(y - 1) * w + (x - 1)]; }

    Term::style Term::Window::get_style(size_t x, size_t y) {
        return m_style[(y - 1) * w + (x - 1)];
    }

    size_t Term::Window::get_w() const { return w; }

    size_t Term::Window::get_h() const { return h; }
    ...
    void Term::Window::set_fg(size_t x, size_t y, fg c) { m_fg[(y - 1) * w + (x - 1)] = c; }

    void Term::Window::set_bg(size_t x, size_t y, bg c) { m_bg[(y - 1) * w + (x - 1)] = c; }
